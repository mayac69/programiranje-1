...
(*Združevanje dveh seznamov v enega*)
...
(*----------------------------------------------------------------------------*
 ...
 imenom `ime_funkcije_tlrec`. Pri repno rekurzivnih funkcijah se funkcija pokliče
 na koncu!
[*----------------------------------------------------------------------------*)
...
[*----------------------------------------------------------------------------*)
(*Tale za negativna števila ne deluje*)
let rec repeat x n = 
    match n with
    | 0 -> []
    | n -> x :: repeat x (n - 1)
...
(* Ta funkcija ni repno rekurzivna, ker najprej kličemo rekurzijo, šele nato spajamo. *)
let rec range n = 
  if n < 0 then []
  else (range (n - 1)) @ [n]


(* Ta funkcija pa je repno rekurzivna: na aux kličemo funkcijo na koncu. *)
...
let map_tlrec funkcija seznam = 
  let rec aux acc preostanek = 
    match preostanek with
    | [] -> reverse acc
    | x :: xs -> aux (funkcija x :: acc) xs
in
aux [] seznam
...
let mapi funkcija seznam = 
  let rec mapi_aux funkcija seznam index =
    match seznam with
    | [] -> []
    | x :: xs -> funkcija index x :: mapi_aux funkcija xs (index + 1)
  in
  mapi_aux funkcija seznam 0
...
let unzip_tlrec seznam_parov = 
  let rec aux seznam_parov acc1 acc2 =
    match seznam_parov with
    | [] -> [(reverse acc1, reverse acc2)]
    | (a, b) :: xs ->  aux xs (a :: acc1) (b :: acc2)
  in
aux seznam_parov [] [] 
...
let rec loop pogoj funkcija x = 
  if pogoj x then loop pogoj funkcija (funkcija x)
  else x
...
let rec fold_left_no_acc funkcija seznam =

    match seznam with
    | [] -> failwith "Seznam ima premalo elementov!"
    | [_] -> failwith "Seznam ima premalo elementov!"
    | x0 :: x1 :: [] -> funkcija x0 x1
    | x0 :: x1 :: xs -> fold_left_no_acc funkcija ((funkcija x0 x1) :: xs)
...
let apply_sequence funkcija x n =
  let rec apply_sequence_aux acc trenutna_vrednost i =
    if i > n then reverse acc 
    else apply_sequence_aux (trenutna_vrednost :: acc) (funkcija trenutna_vrednost) (i + 1) 
  in
  apply_sequence_aux [] x 0
...
let rec filter funkcija seznam =
  match seznam with
  | [] -> []
  | x :: xs -> 
    if funkcija x then x :: filter funkcija xs
    else filter funkcija xs
...
(* Ni repno rekurzivna. *)
let rec exists funkcija seznam = 
  match seznam with 
  | [] -> false
  | x :: xs -> 
    if funkcija x then true
    else exists funkcija xs

(* Je repno rekurzivna. *)
let exists_trilec funkcija seznam =
  let rec exists_aux seznam =
    match seznam with
    | [] -> false
    | x :: xs -> 
      if funkcija x then true
...
let primer_exists_trilec_1 = exists ((<) 3) [0; 1; 2; 3; 4; 5]
...
let first funkcija default seznam =
  let rec first_aux seznam =
    match seznam with
    | [] -> default
    | x :: xs -> 
      if funkcija x then x
      else first_aux xs
  in
  first_aux seznam
