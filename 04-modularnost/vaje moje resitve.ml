...
[*----------------------------------------------------------------------------*)
(* To sta modula, ki iz ničesar zgradita celotno strukturo. Kasneje pa bo še implementacija. *)
module type NAT = sig
  type t
  ...
  (* Seštevanje, imamo dva načina, če damo znak, lahko kličemo med elementoma, sicer kot navadno funkcijo. *)
  val ( + ) : t -> t -> t
  (*  
  val plus : t -> t -> t
  *)
...
module Nat_int : NAT = struct
  ...
  type t = int (* Zato bomo lahko uporabili vse vgrajene funkcije, ki veljajo za int. *)
  let eq x y = x = y
  let zero = 0
  let one = 1
  let ( + ) m n = m + n
  let ( * ) m n = m * n
  let ( - ) m n = if m < n then zero else m - n (* Da bo kolikor toliko smiselno. *)
...
module Nat_peano : NAT = struct
  ...
  let rec eq x y = 
    ...
    let rec ( + ) m n = 
      ...
      | Nic, n -> n
      | m, Nic -> m
      | Naslednik m', Naslednik n' -> Naslednik (Naslednik (m' + n'))
  ...
  let rec to_int n = 
    match n with 
    | Nic -> 0
    | Naslednik n' -> Int.add 1 (to_int n') (* Ker ne moremo klicati + na modulu n, ampak na int. *)
...
let fact_5_int = Nat_int_calc.factorial (Nat_int.of_int 5) |> Nat_int.to_int (* Zapis |> je samo obraten vrstni red klicanja funkcij. *)
