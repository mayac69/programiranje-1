...
let skalarni_produkt v1 v2 = 
  vsota_seznama (List.map2 (fun x y -> x *. y) v1 v2)
...
let norma v1 =
  sqrt (vsota_seznama (List.map (fun x -> x *. x) v1))
...
let projekcija v1 v2 = 
  razteg (skalarni_produkt v1 v2 /. norma v2) v2
...
let razdeli_vrstico niz = 
  let razdeli = String.split_on_char ',' niz in
  (List.nth razdeli 0, List.nth razdeli 1)
...
let pretvori_v_seznam_parov vecvrsticni_niz = 
  let delitev_vecvrsticnega_niza = String.split_on_char '\n' vecvrsticni_niz in
  List.map(fun delitev_vecvrsticnega_niza -> 
    let delitev_vrstic = String.split_on_char ',' delitev_vecvrsticnega_niza in
    (List.nth delitev_vrstic 0, List.nth delitev_vrstic 1)
    )
    delitev_vecvrsticnega_niza
...
let izracunaj_skupni_znesek nakupovalni_seznam cenik  = 
  let moj_seznam = pretvori_v_seznam_parov(nakupovalni_seznam) in
  let trgovinski_seznam = pretvori_v_seznam_parov(cenik) in

  let poisci_ceno izdelek =
    let (opis, cena) = List.find(fun (opis, _) -> opis = izdelek) trgovinski_seznam in
    float_of_string cena
    in 
  let vrednost_izdelkov = List.map(fun (izdelek, kolicina) -> float_of_string kolicina *. poisci_ceno izdelek) moj_seznam in

  List.fold_left (+.) 0. vrednost_izdelkov
